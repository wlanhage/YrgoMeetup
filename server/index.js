...
app.post("/students", async (req, res) => {
  ...
  const validatePassword = (password) => {
    ...
    // validate textfields
    if (!firstname || typeof firstname !== 'string' || !validateFirstName(firstname) || !/^[a-zA-Z]+$/.test(firstname)) {
      res.status(400).send('Invalid first name');
      return;
    }
    if (!lastname || typeof lastname !== 'string' || !validateLastName(lastname) || !/^[a-zA-Z]+$/.test(lastname)) {
  ...
  if ( typeof phone !== 'string' || phone.length !== 0 && phone.length !== 10 || !phone.startsWith("07")) {
